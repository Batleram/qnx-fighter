...
PyMODINIT_FUNC PyInit_smbus(void);
...
SMBus_read_byte(SMBus *self, PyObject *args)
{
    ...
    if (devctl(self->fd, DCMD_I2C_RECV, &rmsg, sizeof(rmsg), NULL) == -1) {
		PyErr_SetFromErrno(PyExc_IOError);
		return NULL;
	}

	return Py_BuildValue("l", (long)rmsg.data);
}
...
SMBus_write_byte(SMBus *self, PyObject *args)
{
    ...
    if (devctl(self->fd, DCMD_I2C_SEND, &smsg, sizeof(smsg), NULL) == -1) {
		PyErr_SetFromErrno(PyExc_IOError);
		return NULL;
	}
...
SMBus_read_byte_data(SMBus *self, PyObject *args)
{
    ...
    // Write the command (control) byte.
    i2c_data_t  smsg = {
        .hdr.slave.addr = addr,
        .hdr.slave.fmt = I2C_ADDRFMT_7BIT,
        .hdr.len = 1,
	...
	return Py_BuildValue("l", (long)rmsg.data[0]);
}
 ...
 */
static int
SMBus_list_to_data(PyObject *list, i2c_data_t *data)
{
	...
	for (ii = 0; ii < len; ii++) {
		...
		data->data[ii + 1] = (uint8_t)PyLong_AS_LONG(val);
#else
		if (!PyInt_Check(val)) {
			PyErr_SetString(PyExc_TypeError, msg);
			return 0; /* fail */
		}
		data->block[ii + 1] = (uint8_t)PyInt_AS_LONG(val);
#endif
	}
...
SMBus_write_block_data(SMBus *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "iiO&:write_block_data", &addr, &cmd,
				SMBus_list_to_data, &smsg))
		return NULL;

    smsg.hdr.slave.addr = addr;
    smsg.data[0] = cmd;
